...
(function() {
    ...
    function createPlayersUI() {
        ...
        // Ajouter l'onglet Joueurs s'il n'existe pas
        ...
        if (!playersContent) {
            ...
            playersContent.innerHTML = `
                ...
                <!-- Liste des joueurs -->
                <div class="list-container" id="players-list-container">
                    <p>Chargement...</p>
                </div>

                <!-- Modal pour ajouter/modifier joueur -->
                ...
                <!-- Modal pour voir les détails (tickets) d'un joueur -->
                <div id="player-details-modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.8); z-index:1000; justify-content:center; align-items:center;">
                    <div style="background:#1e1f36; border-radius:20px; padding:25px; max-width:800px; width:90%; max-height:80%; overflow-y:auto;">
    ...
    async function loadPlayers() {
        ...
        try {
            ...
            // Charger les stats supplémentaires (total mises, gains)
            ...
            for (const p of players) {
                // Récupérer les stats du joueur individuel
                const playerStats = await apiCall(`/api/owner/player-stats/${p.id}`).catch(() => ({ totalBets:0, totalWins:0 }));
                const net = playerStats.totalBets - playerStats.totalWins;
                html += `
                    <tr>
                        <td>${escapeHtml(p.name)}</td>
                        <td>${escapeHtml(p.username)}</td>
                        <td>${escapeHtml(p.zone || '-')}</td>
                        <td class="${p.balance > 0 ? 'profit' : ''}">${p.balance.toLocaleString()} G</td>
                        <td>${playerStats.totalBets.toLocaleString()} G</td>
                        <td>${playerStats.totalWins.toLocaleString()} G</td>
    ...
    function openPlayerModal(playerId = null) {
        ...
        if (playerId) {
            ...
            apiCall(`/api/owner/players/${playerId}`).then(player => {
                ...
                document.getElementById('player-phone').value = player.username;
    ...
    async function deletePlayer(playerId) {
        if (!confirm('Êtes-vous sûr de vouloir supprimer ce joueur ? Toutes ses données seront perdues.')) return;
        try {
    ...
    async function viewPlayerDetails(playerId) {
        ...
        try {
            ...
            if (tickets.tickets && tickets.tickets.length > 0) {
                tickets.tickets.forEach(t => {
                    ticketsHtml += `<tr>
                        ...
                        <td>${t.total_amount.toLocaleString()} G</td>
                        <td class="${t.win_amount > 0 ? 'profit' : ''}">${t.win_amount > 0 ? '+' + t.win_amount.toLocaleString() + ' G' : '-'}</td>
            ...
            // Messages envoyés à ce joueur (optionnel)
            ...
            `;
            contentDiv.innerHTML = ticketsHtml + messagesHtml;
            modal.style.display = 'flex';
        } catch (err) {
    ...
    async function sendMessageToPlayer(playerId, messageText = null) {
        ...
        try {
            ...
            if (document.getElementById('player-details-modal').style.display === 'flex') {
                // Recharger les détails pour afficher le nouveau message
    ...
    // Exposer les fonctions globales pour être utilisées dans les onclick
    ...
    function init() {
        createPlayersUI();
        // S'assurer que switchTab de owner.html charge aussi cet onglet
        ...
        if (originalSwitchTab) {
            ...
            // Si switchTab n'est pas définie (owner.html peut avoir sa propre fonction), on peut simplement appeler loadPlayers quand l'onglet devient actif
        ...
        // Premier chargement si l'onglet est actif
