...
const GameEngine = {
    ...
    generateLottoBetsWithOptions(gameType, num, amount) {
        ...
        // Lotto4 uniquement : si "Transpoze" est activé, on joue aussi la
        // version où les deux moitiés du numéro sont inversées (ex: 4509 ->
        // 0945), en plus du numéro tapé — pour chaque option sélectionnée.
        const numbersToPlay = [cleanNum];
        if (gameType === 'lotto4' && APP_STATE.lotto4Transpose && cleanNum.length === 4) {
            const transposed = cleanNum.slice(2, 4) + cleanNum.slice(0, 2);
            if (transposed !== cleanNum) numbersToPlay.push(transposed);
        }

        for (const numVariant of numbersToPlay) {
            for (let i = 0; i < options.length; i++) {
                if (options[i]) {
                    let displayNum = numVariant;
                    if (gameType === 'lotto4' && numVariant.length === 4) {
                        displayNum = numVariant.slice(0, 2) + '-' + numVariant.slice(2, 4);
                    } else if (gameType === 'lotto5' && numVariant.length === 5) {
                        displayNum = numVariant.slice(0, 3) + '-' + numVariant.slice(3, 5);
                    }
...
// ---------- Transposition lotto4 ----------
function toggleLottoTranspose(gameType) {
    if (gameType !== 4) return; // fonctionnalité uniquement pour lotto4
    APP_STATE.lotto4Transpose = !APP_STATE.lotto4Transpose;

    const chip = document.getElementById('lotto4-transpose-chip');
    const checkbox = document.getElementById('opt-transpose-4');
    if (chip) chip.classList.toggle('active', APP_STATE.lotto4Transpose);
    if (checkbox) checkbox.checked = APP_STATE.lotto4Transpose;

    updateLotto4TransposePreview();
}

function updateLotto4TransposePreview() {
    const preview = document.getElementById('lotto4-transpose-preview');
    if (!preview) return;
    const numInput = document.getElementById('num-input');
    if (!APP_STATE.lotto4Transpose || !numInput || APP_STATE.selectedGame !== 'lotto4') {
        preview.style.display = 'none';
        return;
    }
    const cleanNum = GameEngine.getCleanNumber(numInput.value);
    if (cleanNum.length === 4) {
        const transposed = cleanNum.slice(2, 4) + cleanNum.slice(0, 2);
        preview.innerHTML = `<i class="fas fa-random"></i> Ap jwe tou: ${transposed.slice(0, 2)}-${transposed.slice(2, 4)}`;
