...
const GameEngine = {
    ...
    generateLottoBetsWithOptions(gameType, num, amount) {
        ...
        // Lotto4 : on joue TOUJOURS aussi la version où les deux moitiés du
        // numéro sont inversées (ex: 5298 -> 9852), automatiquement, pour
        // chaque option sélectionnée — pas besoin de l'activer, c'est
        // systématique.
...
// ---------- Transposition lotto4 (systématique, pas d'activation requise) ----------
function updateLotto4TransposePreview() {
    const preview = document.getElementById('lotto4-transpose-preview');
    if (!preview) return;
    const numInput = document.getElementById('num-input');
    if (!numInput || APP_STATE.selectedGame !== 'lotto4') {
    ...
    if (cleanNum.length === 4) {
        const transposed = cleanNum.slice(2, 4) + cleanNum.slice(0, 2);
        preview.innerHTML = `<i class="fas fa-random"></i> Ap jwe tou otomatikman: ${transposed.slice(0, 2)}-${transposed.slice(2, 4)}`;
