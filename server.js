...
async function ensureTables() {
  ...
  // Table global_number_limits (limites globales par numéro, tous tirages)
  await pool.query(`
...
app.post('/api/tickets/save', authenticate, async (req, res) => {
  ...
  try {
    ...
    const drawCheck = await client.query('SELECT active FROM draws WHERE id = $1 AND owner_id = $2', [drawId, ownerId]);
    ...
    const globalBlocked = await client.query(
      'SELECT number FROM blocked_numbers WHERE owner_id = $1 AND global = true',
      [ownerId]
    );
    ...
    const drawBlocked = await client.query(
      'SELECT number FROM blocked_numbers WHERE owner_id = $1 AND draw_id = $2 AND global = false',
      [ownerId, drawId]
    );
    ...
    const globalLimitsMap = new Map(globalLimits.rows.map(r => [r.number, parseFloat(r.limit_amount)]));
    ...
    const blockedLotto3Res = await client.query(
...
// ==================== Routes limites globales par numéro ====================
...
app.post('/api/owner/global-limit', authenticate, requireRole('owner'), async (req, res) => {
  const ownerId = req.user.id;
  ...
  try {
    await pool.query(
      `INSERT INTO global_number_limits (owner_id, number, limit_amount)
       VALUES ($1, $2, $3)
       ON CONFLICT (owner_id, number) DO UPDATE SET limit_amount = EXCLUDED.limit_amount`,
      [ownerId, number, limitAmount]
    );
    res.json({ success: true });
  } catch (err) {
    console.error('❌ Erreur ajout/modif limite globale:', err);
    res.status(500).json({ error: 'Erreur serveur' });
  }
});

app.delete('/api/owner/global-limit', authenticate, requireRole('owner'), async (req, res) => {
  const ownerId = req.user.id;
  const { number } = req.body;
  if (!number) {
...
app.get('/api/owner/dashboard', authenticate, requireRole('owner'), async (req, res) => {
  ...
  try {
    ...
    // Progression des limites globales
    const globalLimitsProgress = await pool.query(
      `SELECT gl.number, gl.limit_amount,
              COALESCE(SUM((bet->>'amount')::numeric), 0) as current_bets,
              (COALESCE(SUM((bet->>'amount')::numeric), 0) / gl.limit_amount * 100) as progress_percent
       FROM global_number_limits gl
       LEFT JOIN tickets t ON t.owner_id = gl.owner_id AND DATE(t.date) = CURRENT_DATE,
            jsonb_array_elements(t.bets::jsonb) as bet
       WHERE gl.owner_id = $1 AND bet->>'cleanNumber' = gl.number
       GROUP BY gl.number, gl.limit_amount`,
      [ownerId]
    );
    ...
    res.json({
      ...
      global_limits_progress: globalLimitsProgress.rows,
