...
app.get('/api/number-limits', authenticate, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT draw_id, number, limit_amount 
       FROM number_limits 
       WHERE owner_id = $1
       ORDER BY (draw_id IS NULL) DESC, draw_id, number`,
      [ownerId]
    );
    res.json(result.rows);
  } catch (err) {
...
app.post('/api/tickets/save', authenticate, async (req, res) => {
  ...
  try {
    ...
    // Récupérer les limites par numéro (spécifiques au tirage + globales)
    const limits = await pool.query(
      `SELECT number, limit_amount 
       FROM number_limits 
       WHERE owner_id = $1 
         AND (draw_id = $2 OR draw_id IS NULL)`,
      [ownerId, drawId]
    );
    const limitsMap = new Map();
    for (const row of limits.rows) {
      const limit = parseFloat(row.limit_amount);
      const current = limitsMap.get(row.number) || Infinity;
      // On prend la limite la plus restrictive (la plus petite) si plusieurs existent
      if (limit < current) {
        limitsMap.set(row.number, limit);
      }
    }
...
// Publication des résultats avec calcul des gagnants (addition de tous les lots gagnants pour la borlette)
        ...
        for (const bet of bets) {
          ...
          if (game === 'borlette' || game === 'BO' || (game && game.startsWith('n'))) {
            if (cleanNumber.length === 2) {
              // Vérifier tous les lots et additionner les gains
  ...
  try {
    const result = await pool.query(
      `SELECT l.draw_id, d.name as draw_name, l.number, l.limit_amount
             ...
             WHERE l.owner_id = $1
             ORDER BY (l.draw_id IS NULL) DESC, l.draw_id, l.number`,
      [ownerId]
    );
    res.json(result.rows);
  } catch (err) {
...
app.post('/api/owner/number-limit', authenticate, requireRole('owner'), async (req, res) => {
  ...
  // Convertir "0" ou "global" en NULL pour les limites globales
  if (drawId === '0' || drawId === 'global' || drawId === 0) {
    drawId = null;
  }
...
app.post('/api/owner/remove-number-limit', authenticate, requireRole('owner'), async (req, res) => {
  ...
  if (drawId === '0' || drawId === 'global' || drawId === 0) drawId = null;

  try {
    await pool.query(
      'DELETE FROM number_limits WHERE owner_id = $1 AND draw_id IS NOT DISTINCT FROM $2 AND number = $3',
      [ownerId, drawId, number]
    );
    res.json({ success: true });
  } catch (err) {
