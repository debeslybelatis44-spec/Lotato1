...
async function ensureTables() {
  ...
  // Table tickets (modifiée : ajout de player_id)
  await pool.query(`
...
app.post('/api/auth/player/register', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO players (name, phone, password, zone) VALUES ($1, $2, $3, $4) RETURNING id',
      [name, phone, hashed, zone || null]
    );
    const playerId = result.rows[0].id;
    const token = jwt.sign({ id: playerId, role: 'player', name, phone }, JWT_SECRET, { expiresIn: '7d' });
...
app.post('/api/auth/player/login', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, name, phone, password, balance FROM players WHERE phone = $1',
      [phone]
    );
    if (result.rows.length === 0) {
...
// Routes pour agents / consultation de joueurs
...
app.post('/api/tickets/save', authenticate, async (req, res) => {
  ...
  try {
    ...
    // Si un playerId est fourni, vérifier le solde et débiter
    let playerName = null;
    if (playerId) {
      const playerRes = await pool.query('SELECT name, balance FROM players WHERE id = $1', [playerId]);
    ...
    const result = await pool.query(
      ...
      [ownerId, agentId, agentName, playerId || null, playerName, drawId, drawName, ticketId, finalTotal, betsJson]
    );
...
// ==================== Routes communes (agents, superviseurs, propriétaires) ====================
...
app.delete('/api/tickets/:id', authenticate, async (req, res) => {
  ...
  try {
    const ticket = await pool.query(
      'SELECT owner_id, agent_id, player_id, date FROM tickets WHERE id = $1',
      [ticketId]
    );
...
// ==================== Routes propriétaire pour gestion des joueurs ====================
app.get('/api/owner/players', authenticate, requireRole('owner'), async (req, res) => {
  ...
  let query = `
    ...
    FROM players p
    WHERE p.id IN (
      SELECT DISTINCT player_id FROM tickets WHERE owner_id = $1
    )
  `;
  const params = [ownerId];
  if (search) {
...
app.get('/api/owner/players/:id', authenticate, requireRole('owner'), async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query('SELECT id, name, phone, zone, balance, created_at FROM players WHERE id = $1', [id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Joueur introuvable' });
    res.json(result.rows[0]);
  } catch (err) {
    console.error(err);
