...
// ==================== Route de sauvegarde des tickets ====================
...
app.get('/api/tickets', authenticate, async (req, res) => {
  ...
  if (req.user.role === 'agent') {
    // Un agent ne voit que ses propres tickets
...
// ==================== Routes pour les joueurs (ajoutées sans casser l'existant) ====================
app.post('/api/auth/player/register', async (req, res) => {
  ...
  try {
    // Récupérer le premier propriétaire disponible
    const ownerRes = await pool.query('SELECT id FROM users WHERE role = $1 LIMIT 1', ['owner']);
    if (ownerRes.rows.length === 0) {
      return res.status(500).json({ error: 'Aucun propriétaire configuré' });
    }
